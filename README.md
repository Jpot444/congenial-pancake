...
For a while it said something else. A prebuffer is fifteen to sixty seconds of
a progress bar and one mechanical sentence, and the sentence had been read
once, so the screen led instead with a rotating deck of prediction-market jokes
and figures out of the firm's own book — shuffled and dealt so nothing
repeated, with the dollar amounts redacted to percentages on the box for every
profile but the owner's, and `buffering ahead` demoted to small print
underneath.

It is gone, and the mechanical sentence leads again. Removed rather than
switched off: `market.js`, the two routes behind it, the settings panel, the
deck and its rotation in both the browser portal and the Shield app, the
`#loaderWhy` small print and the `tune-line` it drew on the television, and
`bufferlines.test.js`. A feature nobody wants is not made better by being kept
where it can be turned back on — the box has one fewer daily fetch, one fewer
credential it could hold, and one fewer thing to be wrong.

`loader.wait(why, detail)` and `loader.show(label, detail)` now do the same
thing. Both stay: `wait` is the long, explain-yourself kind, `show` is the
quick one, and every caller already says which it means.
